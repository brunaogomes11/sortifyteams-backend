...
- [ ] **T006** [P] (BE) `common/`: handler global de erros (ProblemDetail),
...
- [ ] **T008** (APP) Base de rede: axios com interceptor Bearer + fila de
...
- [ ] **T009** (BE) Entidades/repos `usuario`, `refresh_token`, `push_token`.
- [ ] **T010** (BE) `POST /api/auth/registro` (papel JOGADOR|DONO_QUADRA,
...
- [ ] **T011** (BE) Emissão JWT (access 15min) + refresh opaco rotativo em
  tabela (D3): `POST /api/auth/refresh`, detecção de reuso invalida a família.
  Testes de rotação e reuso.
- [ ] **T012** [P] (BE) `POST /api/auth/logout` (revoga refresh) e
  `POST /api/auth/reenviar-solicitacao` (C13 — REJEITADO → PENDENTE).
- [ ] **T013** (APP) Telas: Login, Cadastro (escolha de papel), Aguardando
...
- [ ] **T016** (BE) **`SorteioService` puro + testes unitários na mesma task**
  ...
  persiste os times.
- [ ] **T017** [P] (BE) `POST /api/rachas/{id}/concluir`
  (`duracao_partida_seg?` — C1) e transições de status com testes.
- [ ] **T018** (APP) Fluxo Criar Racha: grid de esportes, adicionar jogadores
  (avulso + busca de usuários), config de sorteio (nº times, balancear).
  Listas virtualizadas (FlashList).
- [ ] **T019** (APP) Tela Resultado do Sorteio (C4 — desenhar pelo design
...
- [ ] **T020** [P] (APP) Cronômetro vinculado ao racha (C1): reaproveitar
  ...
  envia duração.
- [ ] **T021** (APP) Home nova: atalhos + tabs Jogador (Home, Rachas, Quadras
  — C2); lista Meus Rachas.

## Fase 3 — Painel Admin (Fluxo 8)

- [ ] **T022** (BE) Seed de admin (migração ou propriedade de config), form login
  `/admin/login` com sessão + CSRF; layout Thymeleaf base com Tailwind CDN.
- [ ] **T023** (BE) `/admin/solicitacoes`: lista PENDENTES, ações
  Aprovar/Rejeitar (POST com CSRF). Teste MockMvc com sessão.
- [ ] **T024** (BE) Núcleo de notificações (D5): entidade `notificacao`,
...
- [ ] **T025** (BE) Entidades/repos `quadra`, `quadra_foto`, `quadra_horario`;
  CRUD `/api/dono/quadras` (só DONO_QUADRA aprovado, só as próprias — 1:N C12).
- [ ] **T026** (BE) `StorageService` + `LocalDiskStorage` (D4): upload
  multipart de fotos de quadra e de perfil, validação tipo/tamanho, servir via
  URL.
- [ ] **T027** (BE) Grade semanal: `PUT /api/dono/quadras/{id}/horarios`
  (dia_semana, hora_inicio/fim, preço) com validação de sobreposição + testes.
- [ ] **T028** (APP) Navegação do Dono (Minhas Quadras, Agenda — Fluxo 7);
