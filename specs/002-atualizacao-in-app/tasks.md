...
- [x] **T001** (BE) Entidades e repositórios `VersaoRuntime` +
...
- [x] **T002** ⚠ (BE) `VersaoAppService.publicar()` transacional — metadados e
...
- [x] **T003** (BE) Painel `/admin/versoes`: lista do histórico, formulário de
...
- [x] **T004** ⚠ (BE) `ApkStreamService` (D2/D3): leitura por faixa via
...
- [ ] **T005** (BE) `GET /api/app/apk` e `/api/app/apk/{versionCode}`.
  ...
  fase não fecha.
- [x] **T006** (BE) Importar o APK `1.0.0` de
...
- [x] **T007** (BE) Landing (`static/index.html`) e página de convite
...
- [x] **T008** ⚠ (BE) `AtualizacaoService` + `GET /api/app/atualizacao`
...
- [x] **T009** [P] (BE) Liberar `/api/app/**` na cadeia `@Order(1)`, com teste
...
- [x] **T011** (APP) Dependências e configuração do release: `expo-updates`,
...
- [x] **T012** ⚠ (APP) Hook de atualização: consulta a checagem, compara
  ...
  transições.
- [x] **T013** (APP) Tela de atualização (FR-024/FR-026/FR-028): logo, barra de
...
- [x] **T014** ⚠ (APP) Download retomável (C10/FR-009/FR-011):
...
- [x] **T015** (APP) Instalação (FR-013/FR-014): conferir **MD5** do arquivo
...
- [x] **T016** ⚠ [P] (APP) Obrigatória × opcional (FR-015/FR-027): bloqueio sem
...
- [x] **T017** ⚠ [P] (APP) Teto de tempo da checagem (FR-023): estourou, o app
...
- [x] **T019** (BE) Entidades `PacoteConteudo`, `AssetConteudo` (endereçado por
  `hash` único, dando dedup entre pacotes) e `PacoteAsset`; invariante de **um
  pacote ativo por `runtimeVersion`**.
- [x] **T020** (BE) `UpdatesProtocolController` (D1-A): `GET /api/app/manifest`
...
- [x] **T021** (BE) Painel `/admin/conteudo`: upload do bundle exportado,
...
- [x] **T023** (APP) `expo-updates` apontando para o backend, amarrado ao
  `runtimeVersion` da T011.
- [x] **T024** (APP) Conteúdo baixando dentro da tela da Fase 4: progresso
  agregado, retomada sem rebaixar assets já presentes (FR-004/FR-008),
  aplicação e entrada já atualizado (FR-006).
- [x] **T025** ⚠ (APP) Rollback automático (FR-007/C14): pacote que falha ao
...
- [x] **T026** ⚠ (BE) `VersaoExpurgoService` (D7/C23): `@Scheduled` com **cron
...
- [x] **T027** [P] (BE) Painel mostrando quais versões ainda têm binário e
